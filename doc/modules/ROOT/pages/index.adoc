...
* Jandex
    ** supports
        *** Indexing a set of classes | memory efficient representation
        *** Browsing and searching for annotations (even ALSO Java 8 type annotations)
        *** Browsing and searching for declarations and types (even ALSO generic type)
        *** Browsing and searching -- through the -- class & interface inheritance hierarchy
        *** Persisting an index | custom storage efficient format
        *** fast loading of custom storage efficient format
        *** Compatibility -- with -- PREVIOUS API & storage format versions
            **** <<compatibility_promise>>
        *** Execution -- via -- API, command line tool, Ant, and Maven plugin
    ** link:https://javadoc.io/doc/io.smallrye/jandex/latest/index.html[API documentation]

== Requirements

* | Jandex 3.0+,
    ** Java 8+
        *** ❌runtime Java version does NOT affect the Jandex bytecode parsing capabilities❌
            **** == Jandex running | Java 8, can STILL parse Java 17 bytecode

== Getting Help

* link:https://github.com/smallrye/jandex/issues[GitHub Issues]
* link:https://groups.google.com/g/smallrye[SmallRye Google Group]
* link:https://quarkusio.zulipchat.com/[Quarkus Zulip chat]

[[compatibility_promise]]
== Compatibility Promise

* Jandex
    ** follows `X.Y.Z` versioning scheme
        *** `X` == _major_ version
        *** `Y` == _minor_ version
        *** `Z` == _micro_ version
